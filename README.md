...
현재 저장소는 PRD v1.12의 문서 모드 확장 Beta 구현본이다. 신규 문서 작업은 `workflow-v3/thin-core-v1`을 사용해 양쪽 AI의 독립 결과, 지정 통합, 반대 AI의 최종 검증과 필요한 경우에만 수행하는 최종 수정으로 진행한다. 공통 안전 기반, 실행별 안전 진단, 필수 모델·분석 깊이 선택, 불변 스냅샷, 재개 가능한 상태 저장, 키보드 메뉴와 LIVE 진행판, 쟁점 설명·근거 추가·사용자 결정, 통합 복구와 최종 산출물 렌더링을 제공한다.

| 화면 모드 | 내부 ID | 입력 | 결과 | 상태 |
|---|---|---|---|---|
| 1. 컨셉으로 공동 문서 만들기 | `shared-document` | brief 또는 컨셉 C | 공동 문서 C'와 통합 검토 보고서 | 활성 (`workflow-v3`) |
| 2. 두 문서를 하나로 합의하기 | `document-merge` | 문서 A/B | 합의 문서 C와 A/B 출처 추적을 포함한 통합 검토 보고서 | 활성 (`workflow-v3`) |
| 3. 두 문서를 각각 개선하기 | `dual-document` | 문서 A/B | 개선 문서 A'/B'와 차이 비교를 포함한 통합 검토 보고서 | 활성 (`workflow-v3`) |
...
완료 산출물은 실행 폴더의 `final` 아래에 생성된다. workflow-v3 렌더러의 사람용 출력은 다음과 같다.

- 모드 1: 문서 유형별 최종 문서와 `REVIEW_REPORT.md`
- 모드 2: 문서 유형별 합의 최종 문서와 `REVIEW_REPORT.md`
- 모드 3: `document-A-final.md`, `document-B-final.md`와 `REVIEW_REPORT.md`
- 공통: 실제 미결정 질문이 있을 때만 `OPEN_QUESTIONS.md`를 추가한다.

`REVIEW_REPORT.md`는 검토 결과 요약, 주요 결정과 이유, 명시적으로 기록된 채택·거절과 반영 위치를 공통 절로 제공한다. 모드 2에만 문서 A/B 출처 추적, 모드 3에만 개선된 A′/B′ 차이 비교를 추가한다. workflow-v3 단계 결과에 명시적인 채택·거절 또는 반영 위치가 없으면 최종 문서 본문을 다시 해석해 추정하지 않고 기록이 없다고 밝힌다. workflow-v3에서 확인 가능한 `source-trace.md`, `DECISIONS.md`, `comparison.md`, `adoption-log.md` 계열의 정보는 이 보고서 절로 통합하며, 얇은 자동 코어에는 별도 `DEBATE_SUMMARY.md`를 만들지 않는다.

`artifacts.json`은 실제 생성 파일 이름과 SHA-256을 기록하는 프로그램용 내부 인덱스이며 일반 사용자가 읽어야 하는 결과 보고서가 아니다. `REVIEW_REPORT.md`에는 검증·커밋된 단계 상태, 정규 쟁점 기록과 유효한 사용자 결정만 사용하고 원시 stdout/stderr, 프롬프트, 입력·최종 문서 본문, 공급자 원시 출력, 모델 응답 원문과 인증정보를 싣지 않는다. 저장된 workflow-v1/v2 실행은 당시 그래프와 렌더러 계약에 따라 기존 파일명·본문·재개 의미를 계속 유지한다.
...
기존 workflow-v2 v4 실행이 AI 호출 전에 `DF-PROMPT-SIZE-LIMIT`로 멈추면 일반 이어하기를 제공하지 않고 `입력 크기 조정 필요`로 표시한다. 현재 사용자 표면은 작업 메뉴의 `이 작업 복구 준비` 또는 `duoforge recover --run <실행 ID> [--workspace <폴더>]` 하나이며, 정확한 `RECOVER` 확인 뒤 내부 v1/v2 호환 어댑터가 실행당 한 번 v5 입력 투영으로 복구를 준비한다. 이 준비는 공급자를 호출하지 않으며 실제 실행에는 다시 별도의 정확한 `LIVE` 확인이 필요하다. 과거 `repair-prompt`와 `REPAIR`는 공개 사용법이 아니라 저장 실행 호환을 위한 내부 구현 이름으로만 남는다.
...
2026-08-04 기준 PowerShell 7 전체 오프라인 회귀는 `219개 통과, 0개 실패`다. workflow-v1/v2 저장 호환성, workflow-v3 얇은 자동 코어와 단계 결과 계약, 세 작업 유형의 통합 검토 보고서, 조건부 질문 파일과 내부 인덱스, 통합 복구, 사용자 결정 반영, 네 지원 터미널 프레임, 원문·비밀값 비노출을 함께 검증한다. 이 테스트는 실제 Codex·Claude 공급자를 호출하거나 사용자 실행을 재개하지 않는다.
