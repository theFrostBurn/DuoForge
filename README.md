...
현재 저장소는 PRD v1.7의 문서 모드 확장 Beta 1차 완료본이다. 공통 안전 기반, 실행별 안전 진단, 필수 모델·추론 정도 선택, 실행 계획, 불변 스냅샷, 구조화 토론 단계, 재개 가능한 상태 저장, 고정형 토론 진행판, 쟁점 설명·근거 추가·사용자 결정과 최종 산출물 렌더링을 제공한다.
...
인수 없이 실행하면 대화형 PowerShell 7 터미널에서는 환경·구독 상태를 확인한 뒤 번호 선택식 홈 메뉴를 연다. 리디렉션·CI 같은 비대화형 환경에서는 입력을 기다리지 않고 도움말을 출력하며, 하위 명령과 옵션은 자동화·고급 사용을 위한 선택 경로다.
...
대화형 메뉴의 `R → LIVE`와 명시적 `resume --live`는 PowerShell 7 터미널에서 고정형 진행판을 연다. 진행판은 선택한 모드의 실제 단계 장벽을 표시하고, 현재 공급자·작업 단계·대상 문서·경과 시간과 검증·커밋된 최근 결과 최대 3건을 보여준다. 최근 결과는 파일 시각이 아니라 단계 그래프에서 유효한 마지막 3건을 선택한 뒤 오래된 것부터 표시한다. 단계에 문서 계보가 기록되어 있으면 문서 A·문서 B·문서 A/B·공동 문서·합의 문서 C로 구분하고, workflow-v1에 없는 계보는 추정하지 않는다.
...
────────────────────────────────────────────────────────
현재  ● Claude · 작성자 응답 · 공동 문서 · 응답 대기 00:31
최근 확정  ✓ Codex · R1 교차 비평 · 공동 문서
  핵심 제약을 새 쟁점으로 분류했습니다.—새 쟁점 주요 2
최근 확정  ✓ Claude · R1 작성자 응답 · 공동 문서
  제안별 검토 결과를 정리했습니다.—검토 응답 수용 1 · 근거 필요 1
최근 확정  ✓ Codex · R1 공동 문서 합성 · 공동 문서
  승인된 수정만 문서에 반영했습니다.—실제 편집 반영 1 · 미반영 1
```
...
각 확정 항목은 헤더 한 줄과 정제 요약·행동 집계 한 줄, 합계 최대 두 줄로 제한한다. 행동 집계는 0건을 생략하고 `새 쟁점`의 치명적·주요·경미, `검토 응답`의 수용·부분 수용·거부·보류·근거 필요·사용자 결정, `실제 편집`의 반영·부분 반영·미반영·보류를 서로 분리한다. 검토 응답의 수용은 실제 편집 반영이나 전체 합의를 뜻하지 않는다.
...
2026-07-30 기준 오프라인 회귀는 118개다. 기존의 추가 근거 원자성·쟁점 계보·저장 세대·workflow-v1 전체 재개, 의미 기반 대용량 문맥과 확정 피드 회귀에 더해 무인자 홈 메뉴와 엔트리포인트, 진행판 `P` 키 안전 일시정지, ordered dictionary 쟁점 목록 표시, run/local 진단 폴백, writer 이중 실패, process 안전 메타데이터, 단계 재시도·최종 실패, 중단 복구, renderer, UI·메뉴·명시적 CLI 상관관계, 보안 canary, 신규 `logs\` 부재와 레거시 `logs\` 바이트 불변, 생성된 모든 진단 행의 Depth 100 재파싱을 포함한다. 실제 공급자 호출은 수행하지 않는다.
