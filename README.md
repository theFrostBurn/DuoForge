...
2026-07-30 기준 오프라인 회귀는 114개다. 기존의 추가 근거 원자성·쟁점 계보·저장 세대·workflow-v1 전체 재개, 의미 기반 대용량 문맥과 확정 피드 회귀에 더해 run/local 진단 폴백, writer 이중 실패, process 안전 메타데이터, 단계 재시도·최종 실패, 중단 복구, renderer, UI·메뉴·명시적 CLI 상관관계, 보안 canary, 신규 `logs\` 부재와 레거시 `logs\` 바이트 불변, 생성된 모든 진단 행의 Depth 100 재파싱을 포함한다. 실제 공급자 호출은 수행하지 않는다.
