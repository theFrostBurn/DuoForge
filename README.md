...
2026-07-30 기준 오프라인 회귀는 128개다. 기존 회귀에 더해 공통 메뉴의 커서 이동·순환·단축키·비활성 이유·줄 입력 폴백, 정보 블록과 다음 메뉴 사이의 단일 전환 여백, 안전 확인 토큰 분리, heartbeat 스피너, 높이별 개요와 단일 상세 스크롤, 네 지원 프레임의 폭·높이, 공통 page/section/field/list/notice renderer의 hanging indent·문단·ASCII 무색 폴백, 넓은 질문 화면의 적응형 본문 행 배분을 포함한다. 실제 공급자 호출과 `resume --live`는 수행하지 않았다.
