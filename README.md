...
사용자 결정이 필요한 확인 사항이 있으면 질문 카드의 선택값을 기록한 뒤 그 결정을 통합 문서와 최종 검증 입력에 구속력 있게 넣어 다시 실행하고, 필요할 때만 최종 수정한다. 현재 질문에 모두 답하면 답변을 반영해 검토하고, 그 결과 새로운 결정이 필요할 때만 다음 `사용자 확인 단계`를 연다. 최초 단계를 포함해 최대 3회(`1/3`~`3/3`)까지만 질문하며, 더 묻지 않아도 되면 즉시 완료한다. 미답변 질문이 있으면 명시적 `resume --live`도 AI 작업 전에 차단한다. 세 번째 답변 뒤에도 새 결정이 필요하면 네 번째 질문을 열지 않고 `QUESTION_LIMIT_REACHED`로 종료해 질문·확인 사항·결과 문서를 보존한다. 다만 workflow-v3의 과거 결함 때문에 이미 답한 동일 질문이 그대로 반복된 실행은 작업 메뉴의 `복구 준비`와 정확한 `RECOVER`로 공급자 호출 없이 결정 반영 단계를 다시 열 수 있으며, 최종 승인 뒤 새로 확인된 차단 질문도 저장된 AI 권장안이 유효하고 사용자가 그 권장안을 선택한 경우 같은 0-call 복구로 최종 수정 한 번만 열 수 있다. 실제 AI 작업에는 다시 `LIVE`가 필요하며, 이 마지막 최종 수정은 실패해도 자동 재시도하지 않는다. 사용자 화면에는 `사용자 확인을 3번 거친 뒤 멈춤`으로 표시하며 완료나 일부 완료로 오해시키지 않는다.
